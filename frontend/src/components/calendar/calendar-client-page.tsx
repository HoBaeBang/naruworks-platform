...
export function CalendarClientPage() {
  ...
  const navigationDate = getDate(
    ...
    view === "month" ? new Date(year, month - 1, 1) : today,
  );
  ...
  useEffect(() => {
    ...
    async function loadEvents() {
      ...
      try {
        const result = await getCalendarEvents(rangeFrom, rangeTo);
        if (isActive) {
          setEvents(result);
        }
      } catch (cause) {
  ...
  return (
    <main className="min-h-screen bg-[var(--background)] px-6 py-8 text-[var(--foreground)] sm:px-8 lg:px-10">
      <section className="mx-auto flex w-full max-w-6xl flex-col gap-6">
        ...
        {!isLoading && !error && view === "month" && <CalendarMonthView year={year} month={month} events={events} selectedDate={selectedDate} />}
        {!isLoading && !error && view === "week" && <CalendarWeekView anchorDate={navigationDate} events={events} />}
        {!isLoading && !error && view === "day" && <CalendarDayView date={navigationDate} events={events} />}
...
function getViewRange(view: CalendarView, year: number, month: number, date: Date) { if (view === "month") { const from = new Date(year, month - 1, 1); return { from, to: new Date(year, month, 1) }; } if (view === "week") { const from = new Date(date); from.setDate(date.getDate() - date.getDay()); const to = new Date(from); to.setDate(from.getDate() + 7); return { from, to }; } const from = startOfDay(date); const to = new Date(from); to.setDate(from.getDate() + 1); return { from, to }; }
function getNavigationHref(view: CalendarView, year: number, month: number, date: Date, offset: -1 | 1) { const target = new Date(date); if (view === "month") { target.setMonth(target.getMonth() + offset); return getCalendarHref(view, target.getFullYear(), target.getMonth() + 1, target); } target.setDate(target.getDate() + (view === "week" ? offset * 7 : offset)); return getCalendarHref(view, target.getFullYear(), target.getMonth() + 1, target); }
function getTodayHref(view: CalendarView, today: Date) { return getCalendarHref(view, today.getFullYear(), today.getMonth() + 1, today); }
function getCalendarHref(view: CalendarView, year: number, month: number, date: Date) { return view === "month" ? `/calendar?view=month&year=${year}&month=${month}` : `/calendar?view=${view}&date=${formatDate(date)}`; }
function parseView(value: string | null): CalendarView { return value === "week" || value === "day" ? value : "month"; }
...
function viewLabel(view: CalendarView) { return view === "month" ? "월간" : view === "week" ? "주간" : "일간"; }
function rangeLabel(view: CalendarView, from: Date, to: Date) { if (view === "month") return `${from.getFullYear()}년 ${from.getMonth() + 1}월`; if (view === "day") return `${from.getMonth() + 1}월 ${from.getDate()}일`; const lastDay = new Date(to); lastDay.setDate(to.getDate() - 1); return `${from.getMonth() + 1}월 ${from.getDate()}일 - ${lastDay.getMonth() + 1}월 ${lastDay.getDate()}일`; }
