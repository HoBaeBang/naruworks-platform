...
반복 일정은 기본 CRUD가 안정화된 뒤 먼저 구현하며, 공휴일/음력 데이터와 보기 전환은 그 다음 단계로 둔다.
...
기본 진입 화면은 월간 보기이며, 연간/월간/주간/일간 보기를 모두 제공한다.
...
```text
종일 일정 = 사용자가 종료일을 포함 날짜로 입력하고, 서버에는 종료일 다음 날 00:00으로 저장한다.
시간 일정 = 시작 날짜·시간과 종료 날짜·시간을 각각 입력한다.
```
...
반복 일정은 원본 일정 한 건만 DB에 저장한다. 조회할 때 요청 기간 안에 필요한 발생 일정만 계산하여 반환하므로, 종료일이 없는 반복 일정도 무한한 DB 행을 만들지 않는다.

현재 적용한 반복 규칙은 아래와 같다.
...
MONTHLY
= 원본 일자가 없는 달에는 해당 월의 마지막 날에 표시
= 예: 1월 31일 -> 2월 28일 또는 29일 -> 3월 31일

YEARLY
= 2월 29일 일정은 평년에는 2월 28일에 표시

반복 종료일
= 선택하지 않으면 계속 반복
= 선택하면 해당 일시를 넘는 발생 일정은 만들지 않음
```

반복으로 계산된 회차는 원본 일정과 구분해 응답한다. 반복 회차를 수정하거나 삭제할 때 아래 적용 범위를 선택할 수 있다.
...
`이 일정만`은 `calendar_event_exceptions`에 취소 또는 재정의 예외를 저장한다.
`이 일정과 이후 일정`은 기존 시리즈를 직전 회차에서 끝내고, 선택한 회차부터 새 원본 시리즈를 만든다.
`전체 일정`은 기존 원본 일정 전체를 수정하거나 삭제한다.

음력 연간 반복은 `LUNAR_YEARLY` 규칙으로 확장한다. 윤달 기준으로 생성한 일정은 윤달이 없는 해에 같은 평달의 월·일로 표시하며, 음력 30일이 없는 달에는 음력 29일로 보정한다. 다음 달로 넘기는 보정은 사용하지 않는다.
...
구현은 `GET /api/calendar/day-metadata?from=YYYY-MM-DD&to=YYYY-MM-DD` API로 분리한다.
오프라인 한국 음력 변환 라이브러리와 법정 공휴일·대체공휴일 계산 규칙을 사용하며, 화면은 사용자 일정과 별도로 이 데이터를 조회한다.
임시공휴일과 법정공휴일 예외는 `calendar_holiday_overrides` 테이블에 `ADD` 또는 `REMOVE` 작업으로 직접 등록하고, 조회 결과에 병합한다.
...
일요일 = 빨간색
토요일 = 파란색 계열
공휴일 = 요일과 관계없이 빨간색과 공휴일명 표시
음력 = 월간·주간·일간에서 작고 흐린 보조 텍스트로 표시
연간 = 날짜 색상과 공휴일 여부를 우선 표시
```
...
Step 2-1
= 조회 기간 안에서 반복 발생 일정 계산
= 월말 / 윤년 보정 규칙
= 반복 원본 회차 수정

Step 2-2
= 반복 회차별 수정 / 삭제 범위 선택 완료
= 예외 일정과 이후 일정 분리 완료
...
= 회원은 개인 캘린더를 여러 개 만들 수 있고, 생성 시에만 공유 여부를 선택
= 캘린더별 display_color와 is_default를 저장
= OWNER / EDITOR / VIEWER 권한으로 캘린더 경계에서 조회·편집 제어
= OWNER가 역할별 공유 초대 링크를 발급하고 승인 회원이 수락하는 API
= Calendar 화면 왼쪽에서 개인·공유 캘린더를 복수 선택해 동시에 표시
= 새 일정은 별도의 저장 대상 캘린더를 선택하며 VIEWER 캘린더는 대상에서 제외
= 수정 시 원본·대상 모두 편집 권한이 있으면 캘린더 간 일정 이동 가능
= 반복 일정의 한 회차 예외는 다른 캘린더로 이동하지 않음
= OWNER는 공유 캘린더를 삭제할 수 있으며 관련 일정·구성원·초대 링크를 함께 정리
= 개인 캘린더는 하나 이상 유지하며, 기본 개인 캘린더 삭제 시 다른 개인 캘린더를 기본으로 승격
= OWNER 구성원 목록 조회와 참여 회원 제거

원칙
= Google 외부 일정은 OAuth 연결 회원의 읽기 전용 일정이며 공유 대상에 자동 포함하지 않음
= 초대 링크는 7일 뒤 만료되며 DB에는 토큰 원문 대신 SHA-256 해시만 저장
= 링크 수락 전에는 calendar_members 관계를 만들지 않음
= OWNER만 링크 발급·폐기와 참여 회원 제거를 할 수 있음
```
