...
NARU_INITIAL_ADMIN_EMAILS=change_me
```
...
OAuth callback 처리
Google profile 기반 Member 조회/생성
...
```text
환경변수 NARU_INITIAL_ADMIN_EMAILS 사용
```
...
Google 로그인 후 PENDING이면 /join/pending 이동
추천인명 저장
ADMIN이 사용자 승인
...
3. 최초 로그인 시 PENDING 회원 생성
4. api.naruworks.com Cloudflare Tunnel 추가
5. 승인 대기 화면
```
